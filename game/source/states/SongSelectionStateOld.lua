...
function SongSelectionState_old:enter()
    ...
    Conductor.beatHit = function()
        if SongSelectionState_old.beatHit then
            SongSelectionState_old.beatHit(SongSelectionState_old)
        end
    end
...
function SongSelectionState_old:draw()
    --Shake:start()
...
function SongSelectionState_old:update(elapsed)
...
function SongSelectionState_old:beatHit()
    beat = beat + 1
...
function SongSelectionState_old:keypressed(k)
    if k == "down" then
        reset(self)
...
function SongSelectionState_old:leave()
    for key, value in pairs(self.sounds) do
        value:stop()
    end
end

return SongSelectionState_old
